...
{ 
	...
	CMovingObject *pMovingObject = NULL;
	for (int i = 0; i < m_nObjects; ++i)
	{
		XMFLOAT3 Shaft(rand() % 2, rand() % 2, rand() % 2);
		if (Shaft.x == Shaft.y == Shaft.z == 0)
			Shaft.x = 1;
		pMovingObject = new CMovingObject(); pMovingObject->SetMesh(pCubeMesh); //각 정육면체 객체의 위치를 설정한다. 
		pMovingObject->SetPosition(rand() % 100 - 50, rand() % 100 - 50, rand() % 100 - 50);
		pMovingObject->SetRotationAxis(Shaft);
		pMovingObject->SetRotationSpeed(rand()%50 + 10);
